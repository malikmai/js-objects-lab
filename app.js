...
const additionalPokemon = ["Pidgey", "Gastly", "Growlithe"];
additionalPokemon.forEach(name => {
  const selectedPokemon = pokemon.find(pokemon => pokemon.name === name);
  if (selectedPokemon) {
...
const index = game.party.findIndex(pokemon => pokemon.name === "Pikachu");

if (index !== -1) { 
  game.party[index] = pokemon.find(pokemon => pokemon.name === "Raichu");
}
...
console.log("Pokémon in the party:");
...
const pokemonToCatch = pokemon.find(p => p.name === "Snorlax");
...
const pokemonToCatch2 = pokemon.find(p => p.name === "Eevee");
